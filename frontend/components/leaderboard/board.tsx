...
import { useMyRank } from '@/lib/queries';
 ...
 *
 * The rows arrive already rendered from the server. Fetching them from the
 * browser meant an empty card for two seconds on a warm function and thirteen
 * on a cold one, for a query that takes under a millisecond — the wait was all
 * round trip. The state filter works on the rows already in hand, so changing
 * it costs nothing, and only the candidate's own rank is still fetched, which
 * is the one thing that differs per reader.
 */
...
export function Board({
  ...
  /* Filtering in the browser rather than asking the server again: fifty rows
     are already here, and a round trip to remove some of them would undo the
     point of rendering them on the server. Ranks are renumbered so a filtered
     board reads 1, 2, 3 rather than 4, 9, 17. */
  const list = (state ? rows.filter((r) => r.state === state) : rows).map(
    (r, i) => ({ ...r, rank: state ? i + 1 : r.rank }),
  );
