    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        const result = await db.transaction(async (tx) => {
          ...
          /* What the attempt was worth, by the exam's own measures rather
             than by raw speed. See lib/exam-xp.ts — the old rule paid up to
             500 for one good attempt against a ladder that ended at 16,000,
             so the whole thing could be walked in a day, and it paid more for
