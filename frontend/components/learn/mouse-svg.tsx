    ...
    <div className="w-full rounded-xl overflow-hidden border-2 border-pencil/20 shadow-hard-sm bg-paper/50">
      ...
      <div className="flex items-center justify-center p-6">
        <svg viewBox="0 0 200 280" className="w-48 h-64" style={{ maxWidth: '12rem' }}>
          ...
          <rect
            ...
            stroke={scroll ? '#ffd11a' : '#555'}
            strokeWidth={1.5}
            style={{ transition: 'fill 0.1s' }}
          />
          {/* Wheel lines */}
          <line x1={97} y1={58} x2={103} y2={58} stroke={scroll ? '#8a8a84' : '#888'} strokeWidth={1} />
          <line x1={97} y1={62} x2={103} y2={62} stroke={scroll ? '#8a8a84' : '#888'} strokeWidth={1} />
          <line x1={97} y1={66} x2={103} y2={66} stroke={scroll ? '#8a8a84' : '#888'} strokeWidth={1} />
      ...
      {showLegend && (
        <div className="px-4 py-2 bg-pencil/5 border-t-2 border-pencil/10 flex flex-wrap gap-3 justify-center text-xs font-hand text-pencil/50">
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-[#ffd11a]" /> Active</span>
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-[#5a5a57]" /> Button</span>
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-[#4a4a48]" /> Body</span>
        </div>
      )}
    </div>
  );
